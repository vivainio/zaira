...
- [~] Step 3: Split export and wiki synchronization logic into focused modules. In progress.
  ...
  - [ ] `_put_one_file()` still inlines the --status/--diff/--pull/push branches, diagram rendering, image upload, title/folder-move resolution, and label sync -- those still need extracting behind the "compute a plan, then execute it" split the plan calls for.
  - [ ] export.py's fetch/enrich/format/attachment/persist mixing (in `export_ticket()`/`export_to_stdout()`) not yet split.
