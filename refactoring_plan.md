...
- [x] Step 1a: Extract `build_parser()` from `main()` in [zaira/cli.py](zaira/cli.py) (parser construction now side-effect-free and testable).
- [x] Step 1b: Add CLI contract tests ([tests/test_cli_parser.py](tests/test_cli_parser.py)) covering the full command/subcommand tree, help exit codes and stdout routing, `-V`, unknown-command and missing-argument exit codes/stderr routing, representative argument defaults, and `--format` choices.
- [ ] Step 2: Introduce internal error types and structured export/synchronization results behind compatibility adapters.
