...
- [~] Step 4: Centralize Confluence transport, document parsing, and file persistence. In progress.
  ...
  - [ ] Document parsing (front matter across create/wiki/refresh) and file persistence (atomic writes) not yet centralized -- remaining scope of Step 4.
...
- [~] Step 6: Add CI enforcement (Ruff, Windows job). In progress.
  ...
  - [ ] **`uv.lock` was deliberately NOT updated/committed.** This environment's global uv config (`~/.config/uv/uv.toml`) points at a private corporate mirror (`pypi-mirror.services.basware.com`); any local `uv lock`/`uv add`/`uv sync` here rewrites every package URL in uv.lock to that mirror instead of the public PyPI ones the committed lockfile uses (confirmed: HEAD's uv.lock is 100% public pypi.org/files.pythonhosted URLs; a lock regenerated from this machine is 100% the private mirror). That's exactly what the plan's dependency rules warn against ("keep its registry URLs public"). Someone needs to run `uv lock` from a network with real public PyPI access (this sandbox couldn't reach pypi.org directly even with `--system-certs`) and commit the result -- otherwise `pyproject.toml`'s new `ruff` dev dependency and the pinned `select` rule exist without a matching lockfile entry, which `uv sync` will silently reconcile in CI (real internet access there) but leaves local dev environments needing a fresh `uv sync` too.
  - [ ] Windows test job: asked, deliberately skipped for now (CI cost/time, and Windows-specific behavior can't be verified from this environment). Can be added later if wanted.
